...
class EavSetup extends OriginEavSetup
{
    ...
    public function addAttribute($entityTypeId, $attributeCode, array $attr)
    {
        ...
        if (!empty($attr['attribute_set_data'])) {
            foreach ($attr['attribute_set_data'] as $attributeSetData) {
                $setId = $attributeSetData['attribute_set_id'];
                $sortOrder = $attributeSetData['sort_order'] ?? null;
                if (!empty($attributeSetData['group']) || empty($attr['user_defined']) || $setId) {
                    if (!empty($attributeSetData['group']) && $setId) {
                        $attributeGroupCode = $this->convertToAttributeGroupCode($attributeSetData['group']);
                        ...
                        if (empty($groupId)) {
                            $this->addAttributeGroup($entityTypeId, $setId, $attributeSetData['group']);
                        } else {
                            $attr['group'] = $groupId;
                        }
