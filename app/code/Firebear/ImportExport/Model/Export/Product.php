...
class Product extends \Magento\CatalogImportExport\Model\Export\Product implements EntityInterface
{
    ...
    protected function initCategories()
    {
        foreach ($this->_storeIdToCode as $storeId => $storeCode) {
            ...
            $collection->setFlag('firebear_category_collection', true);
    ...
    public function export()
    {
        ...
        if (!empty($this->_parameters[Processor::BEHAVIOR_DATA]['export_by_page']) &&
            ($this->_parameters[Processor::BEHAVIOR_DATA]['file_format'] == 'csv' ||
                $this->_parameters[Processor::BEHAVIOR_DATA]['file_format'] == 'google')
            && $historyType == 'admin') {
            ...
            if (!$isAllStoresExported) {
                ...
                $isAllStoresExported = $this->getAllStoresExported();
                if ($page == $lastPage || $isAllStoresExported) {
                    $this->cacheSave(0, 'export_by_page' . $jobId);
                } else {
            ...
            while (true) {
                ...
                if ($this->isCollectionLastPage($entityCollection, $lastPage, $pageSize)) {
                    break;
                }
            }
     ...
     * @return bool
     */
    protected function isCollectionLastPage($entityCollection, $lastPage, $pageSize)
    {
    ...
    protected function collectMultirawData()
    {
        ...
        $entityCollection->addFieldToFilter(
            'entity_id',
            ['in' => array_unique($this->exportedProductIds)]
        );
        $entityCollection->setStoreId(Store::DEFAULT_STORE_ID);
        ...
        $entityCollection->getSelect()->reset(\Magento\Framework\DB\Select::WHERE);
    ...
    public function addCategoryPosition($collection)
    {
        ...
        $productIds = [];
        $collection->setFlag('firebear_product_collection', true);
    ...
    protected function collectMultiselectValues($item, $attrCode, $storeId)
    {
        ...
        $options = array_intersect_key(
            $this->attributeStoreValues[$attrCode][$storeId],
    ...
    protected function fireloadCollection()
    {
        ...
        if (isset($this->getParameters()['only_admin'])
            ...
            $productIdsForDefaultStore = [];
            foreach (array_keys($this->getStores()) as $storeId) {

                if ($storeId !== Store::DEFAULT_STORE_ID) {
                    $collectionByStore->addWebsiteFilter([
                        $this->_storeManager->getStore($storeId)->getWebsiteId()
                    ]);
                }

                $collectionByStore->addStoreFilter($storeId);
                if ($this->getLastPageExportedStatus($storeId) && empty($productIdsForDefaultStore)) {
                    continue;
                }
                $this->setLastPageExportedStatus($collectionByStore, $storeId);

                if (!empty($productIdsForDefaultStore) && $storeId !== Store::DEFAULT_STORE_ID) {
                    $collectionByStore->addFieldToFilter(
                        'entity_id',
                        ['in' => array_unique($productIdsForDefaultStore)]
                    );
                }

                $collectionByStore->setFlag('firebear_product_collection', true);

                foreach ($collectionByStore as $itemId => $item) {
                    $data[$itemId][$storeId] = $item;
                    if ($storeId == Store::DEFAULT_STORE_ID) {
                        $productIdsForDefaultStore[] = $itemId;
                    }
                }
     ...
     * @return array
     */
    protected function getSwatchesByOptionsId($storeId, $optionIds, $attributeId)
    {
        if (!isset($this->cachedSwatchOptions[$storeId])
            || !isset($this->cachedSwatchOptions[$storeId][$attributeId])
            || empty($this->cachedSwatchOptions[$storeId][$attributeId])) {
            if (!isset($this->cachedSwatchOptions[$storeId])) {
                $this->cachedSwatchOptions[$storeId] = [];
            }
            $this->cachedSwatchOptions[$storeId][$attributeId] = [];
            $swatchCollection = $this->swatchCollectionFactory->create();
            $swatchCollection->addFilterByOptionsIds($optionIds);
            $swatchCollection->addStoreFilter($storeId);
            foreach ($swatchCollection as $item) {
                $this->cachedSwatchOptions[$storeId][$attributeId][$item['option_id']] = $item->getData();
            }
        }

        return $this->cachedSwatchOptions[$storeId][$attributeId];
    }
            ...
            foreach ($stores as $id => $code) {
                ...
                try {
                    ...
                    if ($this->swatchesHelperData->isTextSwatch($attribute)) {
                        ...
                        $swatchOption = $this->getSwatchesByOptionsId(
                            $id,
                            $swatchOptionsIds,
    ...
    public function getFieldsForExport()
    {
        ...
        /**
         * Populate _headerColumns with custom field from FieldPool
         */
    ...
    {
        parent::_initStores();
        if (!empty($this->_storeIdToCode) && !isset($this->_storeIdToCode[Store::DEFAULT_STORE_ID])) {
            $this->_storeIdToCode[Store::DEFAULT_STORE_ID] = Store::ADMIN_CODE;
