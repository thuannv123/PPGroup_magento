...
class Category extends AbstractEntity implements EntityInterface
{
    ...
    protected function getArrEntityCollection()
    {
        ...
        if ((empty($store_ids) && empty($this->_parameters['behavior_data']['store_ids']))
            ...
            foreach ($stores as $store) {
                $storeEntityCollection = $this->_getEntityCollection(true);
                if (!empty($storeEntityCollection)) {
                    $entityCollections[$store->getCode()] = $storeEntityCollection;
                }
            }
        } else {
    ...
    public function export()
    {
        ...
        foreach ($storiesEntityCollection as $storeViewCode => $entityCollection) {
            ...
            while (true) {
                ...
                $rootCategory = $this->objectManager->create(\Magento\Catalog\Model\Category::class);
                $rootCategory->setStoreId($store->getId())->load($rootCategoryId);
                $rootCategoryPath = $rootCategory->getPath();

                if (!empty($rootCategoryPath)) {
                    $entityCollection->addPathFilter($rootCategoryPath);
                }
                ...
                if (!$headerColumnsIsSet) {
                    $writer->setHeaderCols($this->_getHeaderColumns());
                    $headerColumnsIsSet = true;
                }
    ...
    protected function collectRawData($collection)
    {
        ...
        foreach ($collection as $itemId => $item) {
            ...
            $data[$itemId]['parent_id'] = $item->getParentId();
        }
