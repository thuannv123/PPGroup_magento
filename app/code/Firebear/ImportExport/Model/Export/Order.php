...
class Order extends AbstractEntity implements EntityInterface
{
    ...
    * Increment Id Column Name
    ...
    protected $_prefixData = [
        ...
        'magento_rma' => 'rma',
        'magento_rma_item_entity' => 'rma_item',
        'magento_rma_shipping_label' => 'rma_shipping_label'
    ];
    ...
    public function __construct(
        ...
        if ($this->moduleManager->isEnabled('Magento_Rma')) {
            $this->itemsFactory = ObjectManager::getInstance()->get(CollectionFactory::class);
        }
     ...
     * @throws LocalizedException
     */
    protected function prepareRmaItems($entityIds)
    {
        $collection = $this->itemsFactory->create()->addAttributeToSelect(
            '*'
        ...
        foreach ($collection as $item) {
            $data = $item->getData();
            $row = [];
            foreach ($item->getAttributes() as $attributeCode => $attribute) {
                if (isset($data[$attributeCode]) && $attribute->getBackendType() == 'int') {
                    $data[$attributeCode] = $attribute->getSource()->getOptionText($data[$attributeCode]) ?? $data[$attributeCode];
                }
            }
            /**
             * add prefix
            ...
            foreach ($data as $attributeCode => $value) {
                $row['rma_item:' . $attributeCode] = $value;
            }
            $instr = $this->_scopeFields('magento_rma_item_entity');
            $allFields = $this->_parameters['all_fields'];
            if (!$allFields) {
                $row = $this->_changedColumns($row, $instr, 'magento_rma_item_entity');
            } else {
                $row = $this->_addPartColumns($row, $instr, 'magento_rma_item_entity');
            }
            if ($this->_isNested()) {
                $exportData[] = ['rma_item' => $row];
            } else {
                $this->_exportData[$rowId] = array_merge($this->_exportData[$rowId] ?? [], $row);
                $rowId++;
            }
        ...
        if ($this->_isNested() && !empty($exportData)) {
            $this->_exportData[0]['rma_item'] = $exportData;
        }
    }
