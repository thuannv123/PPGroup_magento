...
class Product extends MagentoProduct
{
        ...
        if (!empty($this->_parameters['configurable_switch']) && isset($rowData['product_type'])
            ...
            if ($skuConf) {
                ...
                if (!empty($arrayConf)) {
                    ...
                    $arrayConf[Product\ConfigurationVariations::FIELD_COPY_VALUE]['store_id'] =
                        (int)$this->getRowStoreId($rowData);
                    $arrayConf[Product\ConfigurationVariations::FIELD_COPY_VALUE]['child_sku'] = $rowData['sku'];
    ...
    protected function saveProductsCache($inRows, $upRows)
    {
        ...
        foreach ($upRows as $row) {
            ...
            $sku = $skuById[$id];
            if (isset($this->originalImportRows[$sku])) {
    ...
    public function customFieldsMapping($rowData)
    {
        ...
        if (isset($rowData['additional_images'])) {
            $addImage = [];
            foreach (explode($this->getMultipleValueSeparator(), $rowData['additional_images']) as $rowImage) {
                $addImage[] = trim($rowImage);
            }
            if (!empty($addImage)) {
                $rowData['additional_images'] = implode($this->getMultipleValueSeparator(), $addImage);
            }
        }
    ...
    protected function _parseAdditionalAttributes($rowData)
    {
        ...
        $rowData = $this->getSource()->handleReplacing($rowData);
        return $rowData;
    }
     ...
     * @return array
     */
    public function categoriesMapping($rowData)
    {
        $mappedCategories = [];
        $mappedCategoriesPosition = [];
    ...
    protected function saveConfigurationVariations($data, $existingImages = [])
    {
        if (!empty($data)) {
            ...
            foreach ($data as $skuConf => $elements) {
                ...
                $eavAttributesByStores = $this->prepareDataForConfigurable($elements);
                unset($firstElement[Product\ConfigurationVariations::FIELD_COPY_VALUE]['store_id']);
                unset($firstElement[Product\ConfigurationVariations::FIELD_COPY_VALUE]['child_sku']);
                ...
                if (!empty($fieldCopyValue)) {
                    ...
                    if (!empty($eavAttributesByStores)) {
                        $updateData['eav_attributes_by_stores'] = $eavAttributesByStores;
                    }
                    unset(
    ...
    /**
     * Prepare DataForConfigurable
     *
     * @param array $elements
     * @return array
     */
    protected function prepareDataForConfigurable($elements)
    {
        $firstElement = current($elements) ?? [];
        $simpleValAttr = empty($this->_parameters['copy_simple_value']) ? []
            : array_column($this->_parameters['copy_simple_value'], 'copy_simple_value_attributes');

        $eavAttributesByStores = [];
        foreach ($elements as $key => $element) {
            $attributes = $element[Product\ConfigurationVariations::FIELD_COPY_VALUE];
            $attributeStoreId = $attributes['store_id'];
            if (($attributeStoreId !== Store::DEFAULT_STORE_ID)
                && ($firstElement[Product\ConfigurationVariations::FIELD_COPY_VALUE]['child_sku']
                    == $attributes['child_sku'])) {
                foreach ($attributes as $attribute => $value) {
                    if (!in_array($attribute, $simpleValAttr)) {
                        unset($attributes[$attribute]);
                    }
                }
                $attributes['store_id'] = $attributeStoreId;
                $eavAttributesByStores[] = $attributes;
            }
        }
        return $eavAttributesByStores;
    }
    ...
    public function updateExistingProductNotInImportList(array $_currentRowSkus)
    {
        ...
        if (!empty($actionForExistingProducts) && $actionForExistingProducts !== StockStatusOptions::NOT_SET) {
            if ($actionForExistingProducts == StockStatusOptions::DISABLE_PRODUCT) {
                $productIds = $this->getProductIdsNotInImportListByStoreId(
                    $_currentRowSkus,
                    $actionForExistingProducts
                );
                $this->disableExistingProducts($productIds);
            } elseif ($actionForExistingProducts == StockStatusOptions::OUT_OF_STOCK
                || $actionForExistingProducts == StockStatusOptions::ZERO_QTY
                || $actionForExistingProducts == StockStatusOptions::OUT_OF_STOCK_AND_ZERO_QTY) {
                $productIds = $this->getProductIdsNotInImportList($_currentRowSkus);
                ...
                $this->changeVisibilityForExistingProducts($productIds, $actionForExistingProducts);
            }
        }
     ...
     * @throws Exception
     */
    protected function getProductIdsNotInImportList($_currentRowSkus)
    {
        $productIds = [];
    ...
    /**
     * Get ProductIdsNotInImportListByStoreId
     *
     * @param array $_currentRowSkus
     * @param int $actionForExistingProducts
     * @return array
     * @throws LocalizedException
     */
    protected function getProductIdsNotInImportListByStoreId($_currentRowSkus, $actionForExistingProducts)
    {
        ...
        foreach ($websites as $website) {
            ...
            if ($actionForExistingProducts == StockStatusOptions::DISABLE_PRODUCT) {
                $products->addAttributeToFilter('status', ['eq' => Status::STATUS_ENABLED]);
            }
    ...
    protected function disableExistingProducts($productIds)
    {
        try {
            ...
            if (!empty($productIds)) {
                ...
                $data = $this->updateProductAttributes($productIds, $updateData);
            }
            if (!empty($data)) {
    ...
    /**
     * Change VisibilityForExistingProducts
     *
     * @param array $productIds
     * @param int $actionForExistingProducts
     */
    protected function changeVisibilityForExistingProducts($productIds, $actionForExistingProducts)
    {
        try {
            $data = [];
            if (!empty($productIds)) {
                switch ($actionForExistingProducts) {
                    case StockStatusOptions::CHANGE_VISILIBILITY_TO_CATALOG_SEARCH:
                        $visibility = Visibility::VISIBILITY_BOTH;
                        $message = __('Product visibility with sku %1 is set as \'Catalog,Search\'');
                        break;
                    case StockStatusOptions::CHANGE_VISILIBILITY_TO_SEARCH:
                        $visibility = Visibility::VISIBILITY_IN_SEARCH;
                        $message = __('Product visibility with sku %1 is set to Search');
                        break;
                    case StockStatusOptions::CHANGE_VISILIBILITY_TO_CATALOG:
                        $visibility = Visibility::VISIBILITY_IN_CATALOG;
                        $message = __('Product visibility with sku %1 is set to Catalog');
                        break;
                }
                $updateData = [
     ...
     * @return array
     */
    protected function updateProductAttributes($productIds, $data)
    {
        $result = [];
        ...
        foreach ($productIds as $storeId => $productIdsData) {
            if (!empty($productIdsData)) {
                ...
                $productAction->updateAttributes($productIdsData, $data, $storeId);
            }
        }
        return $result;
    }
    ...
    protected function updateInventoryStockItems(array $productIds, $actionForExistingProducts)
    {
        ...
        if (!empty($updateStockData) && !empty($productIds)) {
            try {
