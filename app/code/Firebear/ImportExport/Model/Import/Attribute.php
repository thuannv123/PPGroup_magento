...
use Firebear\ImportExport\Model\ValidatorChain;
use Firebear\ImportExport\Model\ValidatorChain\StringLength;
...
class Attribute extends AbstractEntity implements ImportAdapterInterface
{
    ...
    /**
     * Current attribute code for importing attribute with options
     *
     * @var string
     */
    protected $currentAttributeCode = '';
        ...
        while ($bunch = $this->_dataSourceModel->getNextBunch()) {
            foreach ($bunch as $rowNumber => $rowData) {
                if (!empty($rowData[self::COLUMN_ATTRIBUTE_CODE])) {
                    $this->currentAttributeCode = $rowData[self::COLUMN_ATTRIBUTE_CODE];
                }
                if ($this->isChildRow($rowData) && empty($this->currentAttributeCode)) {
                    continue;
                }
                /* validate data */
    ...
    protected function _saveAttribute(array $rowData)
    {
        ...
        try {
            $this->_eavSetup->addAttribute($this->_getEntityTypeId(), $this->currentAttributeCode, $rowData);
        } catch (\Exception $exception) {
            $this->addLogWriteln($exception->getMessage(), $this->getOutput(), 'error');
        }
        if (!$this->isChildRow($rowData)) {
            $time = explode(" ", microtime());
            $endTime = $time[0] + $time[1];
            $totalTime = $endTime - $startTime;
            $totalTime = round($totalTime, 5);
            $message = 'recorded attribute with code: %1 .... %2s';
            $this->addLogWriteln(__($message, $this->currentAttributeCode, $totalTime), $this->output, 'info');

            if (!isset($rowData['option'])) {
                $this->_alreadyRecorded[] = $this->currentAttributeCode;
            }
        }
    }
    ...
    {
        if (!empty($this->currentAttributeCode) && !in_array($this->currentAttributeCode, $this->_alreadyDeleted)) {
            $time = explode(" ", microtime());
            $startTime = $time[0] + $time[1];

            $this->_eavSetup->removeAttribute($this->_getEntityTypeId(), $this->currentAttributeCode);
            ...
            $this->_alreadyDeleted[] = $this->currentAttributeCode;
        }
    }
    ...
    {
        if (empty($this->currentAttributeCode)) {
        ...
        $isAllowedLength = ValidatorChain::is(
            trim($this->currentAttributeCode),
            StringLength::class,
            ['min' => $minLength, 'max' => $maxLength]
        );

        if (!preg_match("/^[a-zA-Z]+[a-zA-Z0-9_]+$/", $this->currentAttributeCode)) {
            $errorMessage = __(
        ...
        if (empty($rowData[self::COLUMN_ATTRIBUTE_SET]) && !$entityId && !$this->isChildRow($rowData)) {
        ...
        if (!empty($rowData['is_user_defined'])) {
            $info = $this->_eavSetup->getAttribute($this->_getEntityTypeId(), $this->currentAttributeCode);
    ...
    /**
     * Check if the row is a child row
     *
     * @param $rowData
     * @return bool
     */
    protected function isChildRow($rowData)
    {
        if (empty($rowData[self::COLUMN_ATTRIBUTE_CODE]) &&
            (isset($rowData[self::COLUMN_STORE_ID]) && $rowData[self::COLUMN_STORE_ID] !== null) &&
            (!empty($rowData['frontend_label']) ||
                !empty($rowData[self::COLUMN_ATTRIBUTE_BASE_OPTION]) ||
                !empty($rowData['option']) ||
                !empty($rowData['store_labels']) ||
                !empty($rowData[self::COLUMN_SWATCH_ATTRIBUTE_OPTION]) ||
                !empty($rowData[self::COLUMN_ATTRIBUTE_OPTION]))) {
            return true;
        } else {
            return false;
        }
    ...
    {
        $attributeCode = $this->currentAttributeCode ?? $rowData[self::COLUMN_ATTRIBUTE_CODE];
        $bind = [
     ...
     * @return string|null
     */
    protected function _getExistOptionId($attributeId, $value, $sortOrder = 0, int $storeId = 0)
    {
        $resource = $this->_resource;
        ...
        /** @var $select \Magento\Framework\DB\Select */
        $select = $this->_connection->select();
        $select->from(['eaov' => $optionValueTable], 'eaov.option_id')
            ->joinLeft(
                ['eao' => $optionTable],
        ...
        $optionId = $this->_connection->fetchOne($select);
    ...
    protected function _prepareDataForUpdate(array $rowData)
    {
        ...
        if (!empty($rowData[self::COLUMN_ATTRIBUTE_SET])) {
            $this->prepareAttributeSetData($rowData);
        } else {
        ...
        if (isset($rowData[self::COLUMN_ATTRIBUTE_OPTION])
            ...
            if ($entityId && $value) {
                $optionSortOrder = $rowData[self::COLUMN_ATTRIBUTE_OPTION_SORT_ORDER] ?? 0;
                $optionId = $this->_getExistOptionId($entityId, $value, $optionSortOrder, $storeId);
            }

            $rowData['option'] = [];

            if (!$optionId) {
                $rowData['option']['value']['new'][$storeId] = trim($value);
            } else {
        ...
        if (!empty($rowData[self::COLUMN_ATTRIBUTE_DELETE_VALUES])) {
            if ($this->getBehavior() == Import::BEHAVIOR_APPEND) {
                ...
                foreach ($valuesForDelete as $valueForDelete) {
                    if ($entityId) {
                        $optionId = $this->_getExistOptionId($entityId, $valueForDelete);
                        if (!empty($optionId)) {
     ...
     * @throws LocalizedException
     */
    protected function prepareAttributeSetData(&$rowData)
    {
        $attributeSetParametersSeparator = $this->_parameters['attribute_set_parameters_separator'] ?? '';
        $multiValueSeparator = $this->_parameters['_import_multiple_value_separator'] ?? '';
        if (!empty($attributeSetParametersSeparator) && !empty($multiValueSeparator)) {
            $attributeSetData = explode($multiValueSeparator, $rowData[self::COLUMN_ATTRIBUTE_SET]);
            foreach ($attributeSetData as $data) {
                $data = explode($attributeSetParametersSeparator, $data);
                unset($rowData[self::COLUMN_ATTRIBUTE_GROUP]);
                unset($rowData[self::COLUMN_ATTRIBUTE_GROUP_SORT_ORDER]);

                $attributeSetName = !empty($data) ? trim(array_shift($data)) : null;
                $attributeGroupName = !empty($data) ? trim(array_shift($data)) : null;

                $sorOrder = !empty($data) ? (int)trim(array_shift($data)) : null;
                if ($attributeSetName) {
                    $attributeSetId = $this->_setList[$attributeSetName] ?? $this->createNewSet($attributeSetName);
                }
                if (!empty($attributeSetId)) {
                    $rowData['attribute_set_data'][] = [
            ...
            $attributeSetName = trim($rowData[self::COLUMN_ATTRIBUTE_SET]);
            $attributeSetId = $this->_setList[$attributeSetName] ?? $this->createNewSet($attributeSetName);
