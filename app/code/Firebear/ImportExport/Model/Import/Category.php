...
class Category extends AbstractEntity
{
    ...
    const ERROR_PATH_CONTAINS_NON_EXIST_CATEGORY = 'pathContainsNonExistCategory';
    const ERROR_PATH_CONTAINS_INVALID_HIERARCHY = 'pathContainsInvalidHierarchy';
    ...
    protected $errorTemplates = [
        ...
        self::ERROR_PATH_CONTAINS_NON_EXIST_CATEGORY => "The path contains a non-existent category with entity_id: %s",
        self::ERROR_PATH_CONTAINS_INVALID_HIERARCHY => "The path contains invalid hierarchy"
    ];
                ...
                foreach ($collection as $category) {
                    ...
                    if ($pathSize > 1) {
                        ...
                        for ($i = 1; $i < $pathSize; $i++) {
                            $category = $collection->getItemById((int)$structure[$i]);
                            if ($category) {
                ...
                try {
                    ...
                    if ($category->getId()) {
                        if (isset($rowData['path'])) {
                            $category->setPath($rowData['path']);
                        } else {
                            $category->setPath(
                                $parentCategory->getPath() . self::DELIMITER_CATEGORY . $category->getId()
                            );
                        }
                    ...
                    if (!empty($rowData[self::COL_STORE_NAME])) {
                        ...
                        $this->updateCategoriesByPath($rowPathWithDefaultDelimiter, $rowData);
                    }
    ...
    protected function updateCategoriesByPath($rowPath, $rowData, $entityid = 0)
    {
        ...
        try {
            ...
            $childrenCount = $resource->getChildrenCount($category->getId());
            $category->setChildrenCount($childrenCount);

            $category->save();

            /**
             * Update parent of category if the value has changed
             */
            $currentParentId = $category->getParentId();
            if (isset($rowData[self::COL_PARENT]) &&
                !empty($rowData[self::COL_PARENT]) &&
                strcmp($rowData[self::COL_PARENT], $currentParentId) !== 0) {
                $_category = $this->categoryRepository->get($categoryId);
                $_category->move($rowData[self::COL_PARENT], null);
            }
        } catch (\Exception $e) {
            $this->getErrorAggregator()->addError(
                $e->getCode(),
    ...
    public function validateRow(array $rowData, $rowNum)
    {
        ...
        if (Import::BEHAVIOR_APPEND == $this->getBehavior() || Import::BEHAVIOR_REPLACE == $this->getBehavior()) {
            if (!empty($rowData[self::COL_PATH])) {
                $categoryEntityIds = explode('/', $rowData[self::COL_PATH]);
                $existCategoryIds = array_values($this->categories);
                if (!$entityId) {
                    $entityId = $this->categories[$rowData[self::COL_NAME]] ?? '';
                }
                foreach ($categoryEntityIds as $categoryId) {
                    if ((int) $categoryId !== $entityId && !in_array($categoryId, $existCategoryIds)) {
                        $this->getErrorAggregator()
                            ->addError(
                                self::ERROR_PATH_CONTAINS_NON_EXIST_CATEGORY,
                                ProcessingError::ERROR_LEVEL_CRITICAL,
                                $rowNum,
                                $categoryId
                            );
                ...
                $lastPathCategoryId = $this->getLastArrayElement($categoryEntityIds);
                if ($lastPathCategoryId && $entityId) {
                    if ((int)$lastPathCategoryId !== $entityId) {
                        $this->getErrorAggregator()
                            ->addError(
                                self::ERROR_PATH_CONTAINS_INVALID_HIERARCHY,
                                ProcessingError::ERROR_LEVEL_CRITICAL,
                                $rowNum
                            );
                    }
    ...
    /**
     * Get LastArrayElement
     *
     * @param array $array
     * @return mixed
     */
    protected function getLastArrayElement($array)
    {
        $reverseArray = array_reverse($array, true);
        return reset($reverseArray);
    }
    ...
    {
        $this->rootCategoryUrlData = [];
        $collection = $this->categoryColFactory->create();
        $collection->addAttributeToSelect(self::COL_URL);
        $collection->addAttributeToSelect(self::COL_NAME);
        foreach ($collection as $category) {
            $parentCategories = explode('/', $category->getPath());
            if (count($parentCategories) > 1) {
                $rootParentCategory = $parentCategories[1];
                if (count($parentCategories) == 2) {
                    $this->rootCategories[$category[self::COL_NAME]] = $rootParentCategory;
                } else {
                    $this->rootCategoryUrlData[$rootParentCategory][$category[self::COL_URL]] =
                        $category[self::COL_NAME];
                }
            }
        }
    }
    ...
    protected function findUrlKeyDuplicates($rowData, $rowNum)
    {
        ...
        $urlAlreadyExistForRowRootCategory = $this->urlAlreadyExistForRowRootCategory($rowData);
        ...
        if (isset($rowData[self::COL_URL_PATH]) && isset($rowData[self::COL_URL])
            ...
            if ($urlAlreadyExistForRowRootCategory &&
                (isset($rowData[self::COL_URL]) && !empty($rowData[self::COL_URL])
            ...
            if (isset($rowData[self::COL_URL_PATH]) && empty($rowData[self::COL_URL_PATH])) {
                if ($urlAlreadyExistForRowRootCategory &&
                    ($this->isRootCategory($rowData) && isset($rowData[self::COL_URL])
                    && !empty($rowData[self::COL_URL])
                    && in_array($rowData[self::COL_URL], $this->urlComparableList['url_key'][$storeCode]))
                ...
                if (isset($rowData[self::COL_URL_PATH])) {
                    if ($urlAlreadyExistForRowRootCategory &&
                        (isset($rowData[self::COL_URL_PATH]) && !empty($rowData[self::COL_URL_PATH])
    ...
    protected function isRootCategory($rowData)
    {
        ...
        if (isset($rowData[self::COL_PATH]) &&
            ...
            is_string($rowData[self::COL_PATH])) {
     ...
     * @return bool
     */
    protected function urlAlreadyExistForRowRootCategory($rowData): bool
    {
        $rowRootParentCategoryId = '';
        $rowRootParentCategoryName = '';
        $rowCategoryUrlKey = '';
        $rowCategoryName = '';
        $isAlreadyExistForCurrentRowRootCategory = false;
        ...
        if (!empty($rowData[self::COL_URL])) {
            ...
            $rowCategoryUrlKey = $this->getLastArrayElement(
                explode(self::DELIMITER_CATEGORY, $rowData[self::COL_URL_PATH])
            );
        }
        if (!empty($rowData[self::COL_PATH])) {
            $parentCategories = explode(self::DELIMITER_CATEGORY, $rowData[self::COL_PATH]);
            if (count($parentCategories) > 2) {
                $rowRootParentCategoryId = $parentCategories[1];
            }
        }
        if (isset($rowData[self::COL_NAME])) {
            $parentCategoryNames = explode(self::DELIMITER_CATEGORY, $rowData[self::COL_NAME]);
            $rowCategoryName = $this->getLastArrayElement($parentCategoryNames);
            if (count($parentCategoryNames) > 2) {
                $rowRootParentCategoryName = $parentCategoryNames[1];
            } elseif (count($parentCategoryNames) == 2) {
                $rowRootParentCategoryName = $parentCategoryNames[0];
            }
        }
        if (empty($rowRootParentCategoryId) && !empty($rowRootParentCategoryName)) {
            $rowRootParentCategoryId = $this->rootCategories[$rowRootParentCategoryName] ?? '';
        }
        if (!empty($rowRootParentCategoryId)) {
            if (!empty($this->rootCategoryUrlData[$rowRootParentCategoryId]) &&
                in_array($rowCategoryUrlKey, array_keys($this->rootCategoryUrlData[$rowRootParentCategoryId])) &&
                $this->rootCategoryUrlData[$rowRootParentCategoryId][$rowCategoryUrlKey] !== $rowCategoryName) {
                $isAlreadyExistForCurrentRowRootCategory = true;
            } elseif (!empty($this->rootCategoryUrlData[$rowRootParentCategoryId]) && !empty($rowCategoryName)) {
                $this->rootCategoryUrlData[$rowRootParentCategoryId][$rowCategoryUrlKey] = $rowCategoryName;
            }
        } else {
            if (!empty($this->rootCategoryUrlData[$rowRootParentCategoryId]) && !empty($rowCategoryName)) {
                $this->rootCategoryUrlData[$rowRootParentCategoryId][$rowCategoryUrlKey] = $rowCategoryName;
            }
        }
        return $isAlreadyExistForCurrentRowRootCategory;
    }
