...
class Item extends AbstractAdapter
{
    ...
    public function prepareRowData(array $rowData)
    {
        ...
        $rowData['item:customer_email'] = $rowData['customer_email'] ?? null;
    ...
    protected function _prepareDataForUpdate(array $rowData)
    {
        ...
        $rowData[Entity::COLUMN_CUSTOMER_ID] = $this->getCustomerId(
            $rowData[Entity::COLUMN_CUSTOMER_EMAIL],
            $rowData[Entity::COLUMN_STORE_ID] ?? 0
        );
    ...
    protected function getCustomerIdByEmail($rowData)
    {
        return $this->getCustomerId(
            $rowData[Entity::COLUMN_CUSTOMER_EMAIL],
            $rowData[Entity::COLUMN_CUSTOMER_EMAIL] ?? 0
        );
    }
    ...
    protected function prepareDownloadableLinkData($rowData)
    {
        ...
        $this->nextPurchaseId = $this->nextPurchaseId
            ?: $this->_resourceHelper->getNextAutoincrement(
                $this->_resource->getTableName('downloadable_link_purchased')
            );
        ...
        foreach ($allDownloadableLinkData as $key => $linkData) {
            foreach ($downloadableLinkPurchasedFields as $field) {
                switch ($field) {
                    case 'purchased_id':
                        $downloadableLinkPurchasedData[$key][$field] = $linkData[$field] ?? $this->nextPurchaseId++;
                        break;
                    case 'order_item_id':
                        $downloadableLinkPurchasedData[$key][$field] =
                            $this->itemIdsMap[$rowData['item_id']] ?? $linkData[$field] ?? '';
                        break;
                    case 'order_id':
            ...
            $downloadableLinkPurchasedData[$key]['customer_id'] = $rowData[Entity::COLUMN_CUSTOMER_ID] ?? null;
            foreach ($downloadableLinkPurchasedItemFields as $field) {
                if ($field == 'order_item_id') {
                    $downloadableLinkPurchasedItemData[$key][$field] =
                        $this->itemIdsMap[$rowData['item_id']] ?? $linkData[$field] ?? '';
                } elseif ($field == 'purchased_id') {
                    $downloadableLinkPurchasedItemData[$key][$field] =
                        $downloadableLinkPurchasedData[$key][$field] ?? '';
                } else {
