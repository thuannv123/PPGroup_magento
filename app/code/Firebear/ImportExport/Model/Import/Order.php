...
class Order extends AbstractEntity implements ImportAdapterInterface
{
    ...
    public function __construct(
        ...
        \Firebear\ImportExport\Model\ResourceModel\Order\Helper $resourceHelper,
        Helper $helper,
        ...
        $this->_connection = $context->getResource()->getConnection('sales');
            ...
            if ($this->_order->importData()) {
                ...
                if (!empty($orderIds)) {
                    ...
                    foreach ($orderIds as $incrementId => $orderId) {
                        ...
                        $this->addLogWriteln(
                            __('order with order id: %1', $incrementId),
                            $this->getOutput(),
                            'info'
                        );
                    }
                } elseif (!empty($parameters['generate_shipment_by_track']) ||
                    !empty($parameters['generate_invoice_by_track'])) {
                    $this->_importShipment($orderIds, $orderItemIds);
                }
            }
            $this->_connection->commit();
        } catch (Exception $e) {
            $this->addLogWriteln(__('Error: %1', $e->getMessage()), $this->output, 'error');
     ...
     * @return $this
     */
    protected function alterRowData(string $key, $newValue, &$rowData)
    {
        if (isset($rowData[$key]) && !empty($rowData[$key]) && !empty($newValue)) {
     ...
     * @return bool
     */
    public function addCompanyUserData(array &$rowData)
    {
        ...
        $companyAdminAddress = $companyAdmin->getAddresses();
        $companyAdminAddress ? $companyAdminAddress = current($companyAdminAddress) : false;
        $this
            ->alterRowData('address:email', $companyAdmin->getEmail(), $rowData)
            ->alterRowData('customer_email', $companyAdmin->getEmail(), $rowData)
            ->alterRowData('customer_firstname', $companyAdmin->getFirstname(), $rowData)
            ->alterRowData('customer_lastname', $companyAdmin->getLastname(), $rowData)
            ->alterRowData('address:firstname', $companyAdmin->getFirstname(), $rowData)
            ->alterRowData('address:lastname', $companyAdmin->getLastname(), $rowData);
        if ($companyAdminAddress instanceof AddressInterface) {
            $this
                ->alterRowData('address:city', $companyAdminAddress->getCity(), $rowData)
                ->alterRowData('address:street', $companyAdminAddress->getStreet()[0], $rowData)
                ->alterRowData('address:region', $companyAdminAddress->getRegion()->getRegion(), $rowData)
                ->alterRowData('address:address:region_id', $companyAdminAddress->getRegionId(), $rowData)
                ->alterRowData('address:country_id', $companyAdminAddress->getCountryId(), $rowData);
        }
        if ($companyAdmin->getDefaultShipping()) {
            $this->alterRowData('shipment:shipping_address_id', $companyAdmin->getDefaultShipping(), $rowData);
        }
        if ($companyAdmin->getDefaultBilling()) {
            $this->alterRowData('shipment:billing_address_id', $companyAdmin->getDefaultBilling(), $rowData);
        }
        return true;
    }
    ...
    public function customBunchesData(array $rowData)
    {
        ...
        if (class_exists(\Magento\Company\Api\CompanyManagementInterface::class)) {
            $this->companyManagement = $this->integrationFactory
                ->create(\Magento\Company\Api\CompanyManagementInterface::class);
            if ($this->companyManagement) {
                $this->addCompanyUserData($rowData);
            }
        }

        if (isset($rowData['line_type'])
            && empty($rowData['line_type'])
            && !empty($rowData['increment_id'])) {
            unset($rowData['increment_id']);
        }

        $storeId = $this->getCurrentStoreId($rowData);
        if (empty($rowData['increment_id'])
            && !empty($this->_parameters['use_auto_increment'])
            && (isset($rowData['line_type']) && $rowData['line_type'] === 'order')
            && $storeId
        ) {
            $rowData['increment_id'] = $this->sequenceManager->getSequence(
                \Magento\Sales\Model\Order::ENTITY,
                $storeId
            )->getNextValue();
        } elseif (!empty($rowData['increment_id'])
            && (isset($rowData['line_type'])
                && $rowData['line_type'] === 'order')) {

            $order = $this->orderFactory->create()->loadByIncrementId($rowData['increment_id']);
            if (($storeId) && empty($order->getId())) {
                $this->updateOrderSequnceValue($rowData);
            }
        }

        $addressPrefixes = ['address','billing','shipping'];
        foreach ($addressPrefixes as $addressPrefix) {
            $prefix = $addressPrefix.':street';
            if (!isset($rowData[$prefix]) || empty($rowData[$prefix])) {
                $street = [];
                for ($i = 1; $i <= 4; $i++) {
                    $key = $prefix . $i;
                    if (isset($rowData[$key]) && !empty($rowData[$key])) {
     ...
     * @return int
     */
    protected function getCurrentStoreId($rowData)
    {
        $storeId = null;
        if (!empty($rowData['store_id'])) {
            $storeId = (int)$rowData['store_id'];
        } elseif (!empty($rowData['website_code'])) {
            $storeId = current($this->storeResolver->getWebsiteCodeToStoreIds($rowData['website_code']));
        }
        return $storeId;
    }

    /**
     * Update OrderSequnceValue
     *
     * @param array $rowData
     */
    protected function updateOrderSequnceValue($rowData)
    {
        $incrementId = null;
        $nextIncrementId = null;
        $storeId = $this->getCurrentStoreId($rowData);
        $meta = $this->resourceSequenceMeta->loadByEntityTypeAndStore(
            'order',
            $storeId
        );
        $orderPrefix = $meta->getActiveProfile()->getPrefix();
        $orderSuffix = $meta->getActiveProfile()->getSuffix();
        if (!empty($orderPrefix)) {
            $orderPrefix = preg_quote($orderPrefix, '/');
        }
        if (!empty($orderSuffix)) {
            $orderSuffix = preg_quote($orderPrefix, '/');
        }
        $pattern = '/^' . $orderPrefix . '0*(\d+)' . $orderSuffix . '$/';

        if (preg_match($pattern, $rowData['increment_id'], $matches)) {
            $incrementId = $matches[1];
        }

        $currentSequence = $this->sequenceManager->getSequence(
            \Magento\Sales\Model\Order::ENTITY,
            $storeId
        )->getNextValue();

        if (preg_match($pattern, $currentSequence, $matches)) {
            $nextIncrementId = $matches[1];
        }

        if ($incrementId > $nextIncrementId) {
            $this->_connection->insert($meta->getSequenceTable(), [
                'sequence_value' => $incrementId
            ]);
        }
    }
}
