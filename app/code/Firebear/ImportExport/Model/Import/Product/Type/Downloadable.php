...
class Downloadable extends \Magento\DownloadableImportExport\Model\Import\Product\Type\Downloadable
{
    ...
    protected function saveSampleOptions()
    {
        ...
        foreach ($dataSample as $key => $sampleData) {
            if (empty($sampleData['sample_file']) && empty($sampleData['sample_url'])) {
                unset($dataSample[$key]);
            }
        }
        if (!empty($dataSample)) {
    ...
    protected function saveLinkOptions()
    {
        ...
        foreach ($dataLink as $key => $linkData) {
            if (empty($linkData['link_file']) && empty($linkData['link_url'])) {
                unset($dataLink[$key]);
            }
        }
        if (!empty($dataLink)) {
    ...
    protected function addAdditionalAttributes(array $rowData)
    {
        ...
        return [
            ...
            'links_purchased_separately' => $rowData['links_purchased_separately'] ?? 0
        ];
    }
}
