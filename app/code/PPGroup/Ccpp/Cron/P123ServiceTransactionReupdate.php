...
/**
 * Class P123ServiceTransactionReupdate
 * @package PPGroup\Ccpp\Cron
 */
class P123ServiceTransactionReupdate
{
    const CCPP_METHOD = 'ccpp';
    ...
    {
        $this->log("---------STARTING THE 123SERVICE INQUIRY CRON------------");
        ...
        $orderCollection->getSelect()->joinInner(
            ...
            AND (payment.method = \'' . self::CCPP_METHOD . '\')',
            [
        ...
        $dataConfig = $this->p2c2pApiHelper->receiveMerchantIdAndSecretKeyConfig(false);
        ...
        if ($orderCollection->getSize() > 0) {
            foreach ($orderCollection as $order) {
                ...
                $orderPayload = $this->p2c2pApiHelper->receivePaymentInquiryResponse($order, false);
                ...
                if ($responseCode == '0000' || $responseCode == '000' || $responseCode == '00') {
                    ...
                    $this->log('---------- Order cancel ---------');

                    $orderAdditionalData['payment_status'] = ($responseDesc == 'Payment Expired.') ? "004" : "999";
                    $orderAdditionalData['channel_response_code'] = "03";
                    $orderAdditionalData['channel_response_desc'] = ($responseDesc == 'Payment Expired.') ? "failed (expired)" : "Transaction not found";
        ...
        $this->log("---------ENDING THE 123SERVICE INQUIRY CRON------------");
    }
    ...
    {
        $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/cron_update_123service_order.log');
        $logger = new \Zend_Log();
