...
namespace PPGroup\Integration\Filesystem\Io;
 ...
 */
class OverrideSftp extends Sftp
{
    ...
    public function open(array $args = [])
    {
        ...
        $this->_connection = $objectManager->create(
            '\phpseclib3\Net\SFTP',
            ['host' => $host, 'port' => $port, 'timeout' => $args['timeout']]
        );

        if(!$this->_connection->login($args['username'], $args['password'])){
             // phpcs:ignore Magento2.Exceptions.DirectThrow
            $this->log("Unable to open SFTP connection as " . $args['username'] . ", " . $args['host']);
//             throw new Exception(
...
//             );
    ...
    public function log($logData) {
        $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/sftp_connection_valid.log');
        $logger = new \Zend_Log();
